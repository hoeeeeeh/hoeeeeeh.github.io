---
layout: post
title: "[Javascript] ts/js 로 express 따라 만들기"
author: hoeh
...
## http 모듈 사용안하게끔 바꾸기
node_modules/@types/node 에 보면 http.d.ts 가 자동으로 생성되어있는데 이는 http 모듈이 node.js 의 내장 모듈이라서 그렇다.  
나는 내가 만든 http 모듈의 이름을 마치 진짜 `http` 처럼 사용하고 싶었기 때문에 기존의 진짜 http 를 없애주어야했다.  

여러 가지 방법이 있겠지만 간단하게 `node_modules/@types/node/http.d.ts` 파일을 삭제했다.  

추후 다시 http 로 되돌리려면 @types/node 를 설치해주면 된다.
