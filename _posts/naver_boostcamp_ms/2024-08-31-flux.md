---
layout: post
title: "[네이버 부스트캠프 멤버쉽] flux architecture"
author: hoeh
...
우선 MVC 패턴을 그대로 가져다 쓰기는 힘들다는 크롱님의 이야기를 듣고 조금 더 찾아보았습니다. MVC 패턴은 검색해보면 나오는 이미지가 다 다를 정도로 마치 코에 걸면 코걸이, 귀에 걸면 귀걸이인 것 처럼 MVC 패턴을 쓰기 위해 코드를 욱여넣는 것이 올바른 방법이 맞나? 라는 생각이 들었습니다.
