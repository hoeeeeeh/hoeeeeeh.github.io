...
쿠버네티스 버전을 1.24 이상 버전을 사용하면서, docker 에서 containerd 로 옮겨가게 되었다. 쿠버네티스와 containerd 의 설치 방법은 [k8s-docker-containerd]({{ site.url }}{{ site.baseurl }}/kubernetes/2024/04/06/kubernetes_containerd.html) 에 적어두었다.
...
kubeadm 을 통해 init 을 하고 난 후에, kubelet 의 config 파일을 아래처럼 수정해주면 된다.
![kubelet-config]({{ site.image_path }}/kubelet-config.png)
...
CNI에 대한 자세한 내용은 따로 작성해둔 [Kubernetes-CNI]({{ site.url }}{{ site.baseurl }}/kubernetes/2024/03/21/kubernetes_cni.html) 문서를 참조하자.  
