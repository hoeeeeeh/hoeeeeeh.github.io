...
전체 데이터가 전부 도착하기를 기다리기 보다는, 도착한 데이터 덩어리(chunk)들의 stream 바로 process 한다.  
...
버퍼는 이렇게 데이터(사람)들을 임시로 저장하고 있는 공간이다. 버퍼에 5명이 있다면, 10명이 되었을 때 놀이기구에 태워서 보내버린다.

대기줄에 40명이 존재한다면 놀이기구에 최대 탑승 인원인 30명을 태워서 보내버리고, 남은 10명도 최소 탑승 조건에 부합하므로 다음 놀이기구에 태워서 보내버린다.

혹시 유튜브 영상을 볼 때 버퍼링이 걸려서 로딩이 걸려본 적이 있다면 이해가 조금 더 쉬울 것 같다.  
인터넷 속도가 느려져서 버퍼에 '최소 플레이 시간' 만큼의 데이터가 들어있지 않다면, 데이터가 조금 더 도착할때까지 로딩을 하는 것이다.
