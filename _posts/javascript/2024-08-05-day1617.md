...
- data
  ...
  > 버퍼에 공간이 충분한 경우: 스트림 내부에는 데이터를 임시로 저장하는 버퍼가 있습니다. 이 버퍼에 새로운 데이터를 저장할 공간이 충분할 때, 스트림은 새로운 데이터를 받아들일 준비가 된 상태라고 할 수 있습니다.
  > 연결이 설정된 경우: 네트워크 스트림의 경우, 상대방과의 연결이 정상적으로 설정되어 데이터를 주고받을 수 있는 상태여야 합니다.
  > 읽기 작업이 완료된 경우: 파일 시스템에서 데이터를 읽어오는 스트림의 경우, 이전 읽기 작업이 완료되어 다음 읽기 작업을 수행할 준비가 되어 있어야 합니다.
  > 이러한 상태들인 경우에 발생하는 이벤트인듯 하다.
