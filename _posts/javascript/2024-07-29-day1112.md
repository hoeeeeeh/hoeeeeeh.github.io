...
그런데 여기서 Promise가 제대로 작동하려면 saveNumber 가 존재해야하는(상태가 유지되어야하는) 상황이다.  
...
> Interfaces don’t exist in JavaScript because it a dynamic language, where types are changed so often that the developer may not have even realised.
